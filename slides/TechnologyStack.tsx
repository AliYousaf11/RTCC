...
export function TechnologyStack({ isActive }: SlideProps) {
  return (
    <SlideContainer>
      <div className="max-w-6xl mx-auto w-full">
        ...
        {/* Two-row grid for technology stack cards, compact height */}
        <div className="grid grid-cols-2 md:grid-cols-4 grid-rows-4 md:grid-rows-2 gap-4 mb-8">
          {TECHNOLOGY_STACK.map((tech, index) => (
            <div
              key={index}
              className="bg-white rounded-xl p-3 shadow-lg border hover:shadow-xl transition-all duration-300 hover:-translate-y-1 flex flex-col justify-center"
            >
              <div className="text-center">
                <div className="inline-flex p-2 bg-gray-50 rounded-full mb-2">{React.cloneElement(tech.icon, { className: 'w-6 h-6 ' + tech.icon.props.className })}</div>
                <div className="mb-1">
                  <span className="text-xs font-medium text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full">
                    {tech.category}
                  </span>
                </div>
                <h3 className="text-base font-semibold text-gray-900 mb-1">{tech.title}</h3>
                <p className="text-gray-600 text-xs leading-snug">{tech.description}</p>
              </div>
            </div>
          ))}
        </div>
