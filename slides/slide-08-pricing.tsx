...
export function Slide08Pricing({ isActive }: SlideProps) {
  ...
  return (
    <SlideContainer className=" sm:py-6">
      <div className="max-w-6xl mx-auto w-full h-full flex flex-col">
        <SlideHeader
          title="COMMERCIAL PROPOSAL"
          subtitle="Comprehensive  dd Pricing Structure & Investment Details (Excluding VAT)"
          ...
          >
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              {PRICING_PACKAGES.map((pkg, index) => {
                const styles = packageStyles[pkg.id as keyof typeof packageStyles]
                return (
                  <div
                    key={pkg.id}
                    className={`relative bg-gradient-to-br ${styles.gradient} rounded-2xl border-2 shadow-lg cursor-pointer transition-all duration-500 hover:shadow-2xl hover:scale-105 ${
                      selectedPackage === pkg.id
                        ? `${styles.borderColor} ring-4 ring-opacity-50 ${styles.ringColor} transform scale-105 shadow-2xl`
